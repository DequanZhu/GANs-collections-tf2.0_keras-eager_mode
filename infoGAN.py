...
class Discriminator(tf.keras.Model):
    ...
    def call(self, inputs, training):
        ...
        x = self.conv_2(x)
        x = BatchNorm(is_training=self.is_training)(x, training)
        x = layers.LeakyReLU(alpha=0.2)(x)
        x = layers.Flatten()(x)
        x = self.fc_1(x)
        x = BatchNorm(is_training=self.is_training)(x, training)
...
class infoGAN():
    def __init__(self, args):
        ...
        self.sample_z = np.random.uniform(-1, 1,size=(self.num_examples_to_generate, self.z_dim))
    ...
    def q_loss_fun(self, cont_code_est, cont_code_tg, disc_code_est, disc_code_tg):
    ...
    def train_one_step(self, batch_labels, batch_images):
        ...
        code= np.random.uniform(-1., 1., size=(self.batch_size, self.len_continuous_code)).astype(np.float32)
        # code = tf.random.uniform(shape=(self.batch_size, self.len_continuous_code),minval=-1.,maxval= 1.,dtype=tf.dtypes.float32)
        ...
        with tf.GradientTape() as g_tape, tf.GradientTape() as d_tape, tf.GradientTape() as c_tape:
            ...
            _, d_real_logits, _ = self.d(real_images, training=True)
            ...
            q_loss = self.q_loss_fun(cont_code_est, cont_code_tg, disc_code_est, disc_code_tg)
        ...
        gradients_of_c = c_tape.gradient(q_loss, self.c.trainable_variables)
        ...
        self.q_optimizer.apply_gradients(zip(gradients_of_c, self.c.trainable_variables))
        ...
        for epoch in range(start_epoch, self.epoches):
            for batch_images, batch_labels in self.datasets:
                if int(self.checkpoint.step) == 0 or self.could_load:
                    ...
                    self.test_labels = batch_labels[0:self.num_examples_to_generate]
                    self.test_codes = tf.concat((self.test_labels, np.zeros([self.num_examples_to_generate, self.len_continuous_code])), axis=1)
                ...
                if step % 50 == 0:
                    ...
                    self.batch_z_to_disply = tf.concat([self.sample_z, self.test_codes[:self.num_examples_to_generate, :]], 1)
