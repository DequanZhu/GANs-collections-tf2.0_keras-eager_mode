...
class CGAN():
    def __init__(self,args):
        ...
        self.g = self.make_generator_model(self.z_dim,self.y_dim,is_training=True)
    ...
    def make_generator_model(self,z_dim,y_dim,is_training):
        model = tf.keras.Sequential()
        model.add(DenseLayer(1024,input_dim=z_dim+y_dim))
        model.add(BatchNorm(is_training=is_training))
    ...
    def train(self, load=False):
        self.sample_label=np.random.randint(0,self.y_dim-1,size=(self.num_examples_to_generate))
        ...
        for epoch in range(start_epoch,self.epoches):
            for batch_images, batch_labels in self.datasets:
                ...
                if step % 50 == 0:
                    ...
                    sample_z = np.random.uniform(-1., 1., size=(self.num_examples_to_generate, self.z_dim)).astype(np.float32)
